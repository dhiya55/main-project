"""
EcoTrack Vision — Water Consumption Training (Extended Dataset)
=============================================================
Dataset  : ml/household_water_consumption_extended.csv
Features : household_size, income_level (encoded), property_type (encoded), 
           ...
           has_washing_machine, occupants_children, water_price_per_m3, temperature_c
Target   : daily_consumption_liters

This script uses an extended dataset with household characteristics to 
provide more accurate consumption forecasting.
"""
...
DATA_PATH    = os.path.join(BASE_DIR, 'household_water_consumption_extended.csv')
...
# ── 1. Load Data ───────────────────────────────────────────────────────────
print(SEP)
print("  EcoTrack Vision — Extended Water Consumption Training")
print(SEP)
...
numeric_features = [
    'household_size', 'dwelling_area_sqm', 'num_bathrooms', 
    'occupants_children', 'water_price_per_m3', 'temperature_c'
]
categorical_features = ['income_level', 'property_type']
binary_features = ['has_garden', 'has_dishwasher', 'has_washing_machine']
...
print(f"\n{'-'*68}")
print(f"  {'Algorithm':<22}  {'MAE (L)':>9}  {'RMSE (L)':>9}  {'R2':>8}  {'CV R2':>10}")
print(f"{'-'*68}")
...
for name, pipeline in models.items():
    ...
    print(f"  {name:<22}  {mae:>9.2f}  {rmse:>9.2f}  {r2:>8.4f}  {cv_mean:>10.4f}")
...
summary = {
    ...
    "target": TARGET,
    "dataset": "household_water_consumption_extended.csv",
    "results": results,
...
with open(REPORT_PATH, 'w') as f:
    f.write(f"EcoTrack Vision - Extended Water Consumption Report\n")
...
ax2 = ax1.twinx()
color = 'tab:red'
ax2.set_ylabel('MAE (Liters)', color=color)
ax2.plot(model_names, mae_vals, color=color, marker='o', label='MAE')
ax2.tick_params(axis='y', labelcolor=color)

plt.title('Extended Water Model Comparison')
fig.tight_layout()
